...
class GrbDaemon(Device, SimpleDeviceConfig):
    ...
    def apply_config(self, config: Dict[str, Any]):
        ...
        super()._apply_device_config(config)

        # Apply GCN configuration
        self.gcn_client_id = config.get('client_id')
        self.gcn_client_secret = config.get('client_secret')
        self.gcn_topics = config.get('topics', [])

        # Apply GRB configuration
        self.gcn_domain = config.get('gcn_domain', 'gcn.nasa.gov')
        self.disable_grbs = config.get('disable_observations', False)
        ...
        self.record_not_visible = config.get('record_not_visible', True)
        self.record_only_visible_tonight = config.get('record_only_visible_tonight', False)
        self.min_grb_altitude = config.get('min_altitude', 0.0)
